...
# Point to the EHR folder at the repo root so we can read mappings:
# EHR/data/mappings/indian_drugs.csv
...
try:
    drug_map = pd.read_csv(
        BASE_DIR / "data" / "mappings" / "indian_drugs.csv", dtype=str
    )
    # Normalize brand names: lowercase + strip spaces
    DRUG_MAPPING: dict[str, str] = {}
    for brand, generic in zip(
        drug_map["brand"].str.strip(), drug_map["generic"].str.strip()
    ):
        key = brand.lower()
...
def clean_number(value):
    if not value or str(value).strip() == "" or str(value).lower() in [
        "na",
        "n/a",
        "none",
    ]:
...
def normalize_row(row: dict):
    ...
    if age is None or age < 1 or age > 120:
        birth_date_raw = (
            row.get("birth_date")
            or row.get("Birth Date")
            or row.get("birthDate")
            or row.get("BirthDate")
        )
        if birth_date_raw:
            try:
    ...
    gender_raw = str(
        row.get("Gender") or row.get("gender") or row.get("Sex") or "unknown"
    ).strip().upper()

    # Gender
    gender = "U"
    if gender_raw and (
        gender_raw.startswith("F") or gender_raw == "FEMALE" or gender_raw == "2"
    ):
        gender = "F"
    elif gender_raw and (
        gender_raw.startswith("M") or gender_raw == "MALE" or gender_raw == "1"
    ):
        gender = "M"
    ...
    if bp_systolic is None or bp_diastolic is None:
        bp_raw = str(
            row.get("BP") or row.get("bp") or row.get("Blood Pressure") or ""
        )
    ...
    return {
        ...
        "source_file": row.get("_source_file", "uploaded_csv"),
    }
