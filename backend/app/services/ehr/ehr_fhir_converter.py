...
# Keep a BASE_DIR pointing to the EHR folder for consistency with the
# standalone pipeline layout, even though this backend variant writes
# FHIR JSON to memory instead of disk.
BASE_DIR = Path(__file__).resolve().parents[3] / "EHR"


def save_fhir_bundle(patient_id: str, patient_data: dict) -> str:
    """
    Build a FHIR Bundle for the patient and return it as a JSON string.

    This is adapted from the standalone pipeline version, but instead of
    writing to disk it returns the serialized JSON for upload to Firebase.
    """
    # Patient resource
    patient = Patient.model_construct(
        id=patient_id,
        gender=patient_data["gender"].lower()
        if patient_data.get("gender") in ["M", "F"]
        else "unknown",
        birthDate=f"{datetime.now().year - patient_data.get('age', 50)}-01-01",
    )
    ...
    def create_quantity_observation(
        loinc_code: str,
        display: str,
        value: float,
        unit: str,
    ...
    if meds_text.strip():
        ...
        for med in med_list:
            med_stmt = MedicationStatement.model_construct(
                id=str(uuid.uuid4()),
                status="active",
                medication=CodeableConcept.model_construct(text=med),
                subject=Reference.model_construct(reference=f"Patient/{patient_id}"),
                effectiveDateTime=datetime.now().isoformat(),
            )
            entries.append(
    ...
    return json.dumps(bundle.model_dump(by_alias=True, exclude_none=True), indent=2)
